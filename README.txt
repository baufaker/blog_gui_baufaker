Histórias:

OK login admin;
OK dashboard admin;
OK função criar novo post;
OK página de ver todos os meus posts;
OK guardar sessions com redis;
OK função de logout;
OK função editar post;
OK função deletar post;
OK salvar e mostrar data de criação do post na home;
OK função de criar categorias;
OK função de editar categorias;
OK função de deletar categorias;
- adicionar categorias a cada post;
- inserir comentários nos posts;
- fazer deploy para o heroku com mongolab e redis;
- limitar posts que são mostrados na homepage com plugin de "ver mais";
- frontend home page;

/*********************/

OK [BUG] Editar post não funciona se o post tiver quebra de linhas;
OK [BUg] O post não está sendo renderizado com a formatação que foi criado. Tudo está sendo renderizado em uma linha;